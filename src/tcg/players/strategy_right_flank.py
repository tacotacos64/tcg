...
class RightFlankExpansionist(Controller):
    """
    右翼制圧戦略 (Right Flank Expansion Strategy):
    - SecureHomeExpansionistをベースにする。
    - 右側（2, 5, 8, 11）の制圧を重視する。
    """
    ...
    # 要塞の重要度（接続数と位置に基づく）
    # 上側(0,1,2)が自陣の場合の重要度
    # 右側重視 (Right: 12, Center: 12) - バランス調整
    FORTRESS_IMPORTANCE_UPPER = {
        0: 2, 1: 4, 2: 12,     # 上側エリア
        3: 4, 4: 12, 5: 12,    # 中央上
        6: 4, 7: 12, 8: 12,    # 中央下
        9: 2, 10: 4, 11: 12    # 下側エリア
    }
    
    # 下側(9,10,11)が自陣の場合の重要度（反転）
    # 実際にはflip_board_viewによりUPPERしか使われない可能性が高いが、念のため定義
